...
RUN git clone https://github.com/googleapis/googleapis \
  && cd googleapis \
  && git checkout 497074446009147599499db18141927bc649a561 \
  && cd .. \
  && rm -rf /googleapis/.git/
RUN git clone https://github.com/googleapis/toolkit \
  && cd toolkit/ \
  && git checkout eb3b0cefae5e5eac120d90fae6003c7f7ce7cccc \
  && cd .. \
  && rm -rf /toolkit/.git/
...
RUN if [ "$install_artman_from_source" = true ]; then pip3 install -e /artman; else pip3 install googleapis-artman==0.5.7; rm -r /artman; fi

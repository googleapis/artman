...
The test currently assumes that artman and googleapis folders are under the same
parent folder.
"""
      ...
      for lang in SUPPORTED_LANGS:
        if '%s:' % lang in content:
          if generate_grpc_library(filename, lang):
            failure.append(
              "Failed to generate grpc %s library using %s config."
              % (lang, filename))
          if generate_gapic_library(filename, lang):
            failure.append(
              "Failed to generate gapic %s library using %s config."
              % (lang, filename))
  if failure:
...
def generate_grpc_library(artman_config, lang):
  grpc_pipeline_args = [
    ...
    "execute_pipeline.py",
    '--config',
     '%s,../googleapis/gapic/lang/common.yaml' % artman_config,
    '--language',
    lang,
    'GrpcClientPipeline']
